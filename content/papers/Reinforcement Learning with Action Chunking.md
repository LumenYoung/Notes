...
1.  **Behavior Policy Training**: It first trains a behavior cloning flow policy, denoted as `fξ(·|s)`, using a flow-matching objective on the offline data. This policy learns to approximate the distribution of action sequences found in the dataset.
2.  **Best-of-N Sampling**: Instead of learning a separate policy, QC uses "best-of-N" sampling to implicitly enforce the behavior constraint. At each step, it samples `N` action chunks from the learned behavior policy `fξ(·|s)`.
