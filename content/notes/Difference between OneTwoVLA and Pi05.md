---
id: Difference between OneTwoVLA and Pi0.5
aliases: []
tags: []
date: "2025-10-22"
...
The similarities stop at the here, however: the two papers adopt opposite training philosophies and opposite inference schedules for achieving the decomposition.  
Below are the key differences, extracted from the training & inference details each paper provides.
...
- At run-time the model itself decides whether the current situation calls for another bout of reasoning; if yes it outputs the textual rationale, otherwise it directly emits actions .
...
## Low-level action head / action representation

OneTwoVLA  
- Continuous 6-DoF end-effector poses + gripper width are produced with **flow-matching** (same as π0) **conditioned on the latest reasoning paragraph R**.  
- Reasoning content is re-injected into every action-decoding step so the low-level head “knows” the human-written plan .

π0.5  
- Two-stage:  
  (i) produce sub-task sentence ˆℓ (text tokens),  
  (ii) feed ˆℓ as extra text tokens to the **action expert** that outputs a 50-step continuous chunk via **flow-matching**.  
- Action expert is a **separate 300 M-parameter transformer** whose layers do **not** attend to the main VLM layers; only a one-way cross-attention from action expert to VLM features is used .
